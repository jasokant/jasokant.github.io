...
realm_components.directive('playBackComponent', ['$timeout', '$http', '$q', 'RobotService', 'RepoService', function($timeout, $http, $q, RobotService,RepoService) {
    ...
    return {
        ...
        {
            RepoService.getSessionsForDeviceOwner("Device-1");
...
//------------------------------------------------------------------------
            ...
            {
                label=103;
                var command=RepoService.getFromLocation("DeviceCommand","DeviceCommand-"+label);
                command.then(
